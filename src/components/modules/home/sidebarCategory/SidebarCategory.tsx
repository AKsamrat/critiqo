...
const CategorySidebar = () => {
  ...
  const getIcon = (categoryName: string): LucideIcon => {
    ...
    return (iconMap[key] as LucideIcon) ?? iconMap["default"];
  };

  return (
    <aside className="w-44  bg-gradient-to-r from-blue-100 to-gray-50 rounded-tl-2xl">
      <h2 className="font-bold  bg-[#35A09F] py-1.5 pl-1.5 rounded-tl-2xl ">CATEGORIES</h2>
      <ul className="py-2">
        {categories.map((category) => {
          const IconComponent = getIcon(category.name);
          return (
            <li key={category.id} className="border-b border-gray-100 last:border-b-0  hover:scale-110  duration-700">
              <Link
